...
class URController(ArmController):
    ...
    def move_L_sync(self, position, velocity = None, acceleration = None, solution_space='Space'):
        velocity = self._v if velocity is None else velocity
        acceleration = self._a if acceleration is None else acceleration
        if solution_space == 'Joint':
            self.rtde_C.moveL_FK(position, velocity, acceleration,async = True)
        elif solution_space == 'Space':
            if len(np.array(position).shape) == 1 and len(position) == 6:
                self.rtde_C.moveL(position, velocity, acceleration,async = True)
            elif len(np.array(position).shape) == 2 and len(position[0]) == 9:
                self.rtde_C.moveL(position,async = True)
        return True

    def speed_L(self, pos_speed=None, acceleration=0.5, time=0.0):
        self.rtde_C.speedL(pos_speed, acceleration, time)
        return True

    def speed_J(self, joints_speed, acceleration=0.5, time=0.0):
        self.rtde_C.speedJ(joints_speed, acceleration, time)
        return True
    def stop_speed(self):
